...
**Файл:** `Services/OnnxPredictionService.cs` (метод `BuildFeatures`). Воспроизводит на стороне клиента признаковый конвейер, идентичный Python-этапу обучения (раздел 5.4.1). Идентичность подтверждена расхождением прогнозов LightGBM (Python) и ONNX Runtime (C#) менее 0,001 кг на тестовой выборке.
